  ...
  const handleAddToBasket = async (event: React.FormEvent) => {
    ...
    if (authToken) {
      ...
      const existingItemIndex = basket.findIndex(
        (item) =>
          item.foodSelectedIndex === foodSelectedIndex &&
          item.foodSelectedList === foodSelectedList
      );
      ...
      <div className="col" id="containerFoodCardInside">
        ...
        <div className="row-md-3" id="containerContentCard">
          ...
          <form onSubmit={handleAddToBasket}>
            <div className="mb-3">
              <label htmlFor="quantity" className="form-label" id="imageCard">
                Quantity
              </label>
              <input
                type="number"
